...
def sync(user, pas, userid, rootDir, enc_type, enc_pas, server_url):
    ...
    payload = {'username': user, 'password': pas, 'csrfmiddlewaretoken': csrf_tok}
    rs = s.post(server_url+'/accounts/login/', payload)
    r7= s.post(url=server_url + '/user/' + user + '/userdata/', data={'username': user})
    if r7.ok:
        r6 = s.get(url=server_url + '/user/' + user + '/allfiles/' + str(p.name), data={'owner': int(userid)})
        dat6 = r6.json()
        enc_ty = ''
        for pat in dat6:
            fi=pat['pathLineage']
        ...
        if enc_ty=='' or enc_ty == enc_type:
            ...
            g.close()
            client_server='c'
            print('Deleting on client ' + rootDir)
            progress_bar(rootDir, {}, {}, 7, s)
        # client_server = input('Change files on client or server? (c or s): ')
...
>>>>>>> b4cf29d0e5502d25119d6614a1b89e02dfacdd1b
            else:
                file_dir_list.append(dirname + '/' + relDir1 + '/')
            for fileName in files:
                relDir = os.path.relpath(dir_, rootDir)
                ...
                r2 = s.get(str(
                    server_url + '/user/' + user + '/contents/' + dirname + '/' + relFile) + '.' + enc_type + 'en' + '/',
                    ...
                    if dicti['md5code'] != md:
                        if client_server == 's':
                            ...
                            if not enc_type == 'arc':
                                ...
                                content = ast.literal_eval(dat['fileContent'])
                                # print(content)
                                # print(type(content))
                        ...
                        if i == len(dirlist) - 1:
                            ...
                            if not r2.ok:
                                if client_server == 's':
                                    ...
                                    progress_bar(complete_path[:-1], {}, {}, 3, s)
                        else:
                            pat2 = ''
                            for j in range(i):
                            ...
                            if not r2.ok:
                                if client_server == 's':
                                    ...
                                    print('Deleting from client ' + pat + str(dirlist[i]) + '/')
                                    progress_bar(str(os.path.join(rootDir, pat2)), {}, {}, 7, s)
                                    break
...
<<<<<<< HEAD
                    r5 = s.get(server_url + '/user/' + user + '/data/' + fil,
                               data={'owner': int(userid), 'username': user, 'password': pas})
                    dat5 = r5.json()
                    if not enc_type == 'arc':
                        content = str(dat5['fileContent']).encode()
                    else:
                        content = ast.literal_eval(dat5['fileContent'])
                        # print(content)
                        # print(type(content))
                    ...
                    if fil[-7:-1] == str('.' + enc_type + 'en'):
                        ...
                        patn = os.path.join(rootDir, fil_pat)
                        progress_bar(patn, enc_pas, enc_type, 6, content)

        r8 = s.delete(url=server_url + '/user/' + user + '/userdata/', data={'username': user})

    else:
        print('sync is busy.. please try again later')
=======
...
if dicti['md5code'] != md:
    if client_server == 's':
        ...
        # print(str(
        #     server_url + '/user/' + user + '/data/' + dirname +  '/' +'.' + enc_type + 'en' + relFile) + '/')
        progress_bar(str(import base64
import hashlib
import os
import sys
import requests
